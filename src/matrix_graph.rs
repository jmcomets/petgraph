...
// Should optimize the following, in order of preference: contains(), push(), pop().
type FreeList<T> = IndexMap<T, ()>;

#[derive(Clone)]
struct IdStorage<T> {
    elements: Vec<Option<T>>,
    upper_bound: usize,
    removed_ids: FreeList<usize>,
}
...
impl<T> IdStorage<T> {
    fn with_capacity(capacity: usize) -> Self {
        IdStorage {
            ...
            upper_bound: 0,
            removed_ids: FreeList::new(),
        }
    }

    fn add(&mut self, element: T) -> usize {
        let id = if let Some((id, _)) = self.removed_ids.pop() {
            id
        } else {
...
impl<'a> Iterator for IdIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        // skip removed ids
        while self.removed_ids.contains_key(current) && *current < self.upper_bound {
            *current += 1;
        }
